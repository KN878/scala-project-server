...
trait ShopValidationAlgebra[F[_]] extends ValidationResultLib[F]{
  ...
  def matchOwner(shopId: Option[Long], ownerId: Option[Long]): ValidationResult[ShopValidationError, Unit]

  def ownerHasEnoughMoney(ownerId: Option[Long], amount: Float): ValidationResult[ShopValidationError, Unit]
}
