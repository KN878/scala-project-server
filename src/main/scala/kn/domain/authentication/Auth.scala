...
object Auth {
  ...
  private def _shopOwnerOnly[F[_], Auth](
      implicit F: MonadError[F, Throwable],
  ): BasicRBAC[F, Role, User, Auth] =
    BasicRBAC[F, Role, User, Auth](Role.ShopOwner, Role.Admin)

  def shopOwnerOnly[F[_], Auth](
      pf: PartialFunction[SecuredRequest[F, User, AugmentedJWT[Auth, Long]], F[Response[F]]],
  )(implicit F: MonadError[F, Throwable]): TSecAuthService[User, AugmentedJWT[Auth, Long], F] =
    TSecAuthService.withAuthorization(_shopOwnerOnly[F, AugmentedJWT[Auth, Long]])(pf)
  ...
  private def _customerOnly[F[_], Auth](
      implicit F: MonadError[F, Throwable],
  ): BasicRBAC[F, Role, User, Auth] =
  ...
  def customerOnlyHandler[F[_], Auth](
    ...
    TSecAuthService.withAuthorizationHandler(_customerOnly[F, AugmentedJWT[Auth, Long]])(
      pf,
      onNotAuthorized.run,
    )
}
